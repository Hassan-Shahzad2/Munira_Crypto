...
def detect_fraud(wallet_data):
    ...
    if total_tx > 0:
        if dust_percentage > 50 and dust_count > 20:
            risk_score += 45
            reasons.append(f"🔴 CRITICAL: {dust_count} dust transactions ({dust_percentage:.0f}% of total) — Address poisoning detected")
            ...
            risk_score += 15
            reasons.append(f"NOTICE: {dust_count} dust transactions ({dust_percentage:.0f}% of total) — Monitor activity")
    ...
    if outgoing_tx > 100 and unique_outgoing > 50:
        risk_score += 40
        reasons.append(f"🔴 CRITICAL: {outgoing_tx} outgoing transactions to {unique_outgoing} unique addresses — Mixer/wash trading pattern")
    ...
    if balance < 0.001 and total_tx > 20:
        risk_score += 25
        reasons.append("⚠️ Active wallet with near-zero balance — Pass-through address detected")
    
    max_amt = wallet_data.get('max_tx_amount', 0)
    if network == 'bitcoin':
        if max_amt > 100:
            risk_score += 25
            reasons.append(f"LARGE: Single transaction of {max_amt:.2f} BTC — Unusual activity")
        elif max_amt > 10:
            risk_score += 10
...
def check_wallet():
    try:
        ...
        dust_pct = real_data.get('dust_percentage', 0)
        outgoing_pct = (real_data.get('outgoing_tx_count', 0) / max(real_data['total_tx'], 1)) * 100
        ...
        return jsonify({
            ...
            'dust_percentage': round(dust_pct, 1),
            'outgoing_percentage': round(outgoing_pct, 1),
...
def agent():
    try:
        ...
        system_prompt = (
            "You are Luna, a friendly and knowledgeable blockchain security assistant. "
            "IMPORTANT FORMATTING RULES: "
            "1. NEVER use markdown like **bold** or *italic* - just use plain text. "
            "2. Use line breaks (new lines) to separate different points and ideas. "
            "3. Use emojis at the start of lines for visual organization. "
            "4. Keep paragraphs short and readable. "
            "5. Use bullet points with dashes or emojis, not asterisks. "
            "6. Never use double asterisks or any markdown formatting. "
            "Help users understand cryptocurrency wallet risks, scams, dusting attacks, "
            "and blockchain security in a warm, approachable way. Keep answers helpful but concise. "
            "Never give financial advice."
        )
        if context:
            system_prompt += f"\n\nCurrent wallet scan context: {context}"
        ...
        if res.status_code == 200:
            ...
            # Clean any remaining markdown bold markers just in case
            reply = reply.replace('**', '').replace('*', '')
            # Convert double newlines to HTML line breaks
            reply = reply.replace('\n\n', '<br><br>').replace('\n', '<br>')
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 60)
    print("🌸 LunaShield — AI-Powered Crypto Security (ETH + BTC)")
    print("=" * 60)
    ...
    if groq_key:
        print(f"🤖 Luna AI   : ✅ Configured")
    else:
        print("🤖 Luna AI   : ⚠️ NOT CONFIGURED")
    
    print("🟠 Bitcoin   : Using Blockstream API (free, no key needed)")
    
    print(f"🌐 Server    : http://localhost:{port}")
    print("=" * 60)
    print("\n✨ Welcome to LunaShield — Your Friendly Security Guardian!")
